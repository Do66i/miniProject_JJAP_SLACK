...
router.get("/workspaces/:workspace/channels/:channel/unreads", isLoggedIn, async (req, res, next) => {
  try {
    const workspace = await Workspace.findOne({
      where: { url: req.params.workspace },
      include: [
...
router.post("/workspaces/:workspace/channels/:channel/chats", isLoggedIn, async (req, res, next) => {
  try {
    const workspace = await Workspace.findOne({
      where: { url: req.params.workspace },
      include: [
    ...
    io.of(`/ws-${workspace.url}`).to(`/ws-${workspace.url}-${channel.id}`).emit("message", chatWithUser);
    res.send("ok");
  } catch (error) {
    next(error);
  }
...
router.delete("/workspaces/:workspace/channels/:channel/members/:id", isLoggedIn, async (req, res, next) => {
  try {
    const workspace = await Workspace.findOne({
      where: { url: req.params.workspace },
      include: [
